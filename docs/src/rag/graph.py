...
两种用法：
1. 模块级（向后兼容）：`graph.graph` / `graph.run_rag(...)` —— 绑定环境配置。
2. SDK 实例：`build_graph(runtime=LLMRuntime, store=VectorStore)` —— 每个
   RAGClient 一份独立图，绑定自己的模型客户端与向量库。
"""
...
def build_graph(
    ...
    store_: VectorStore | None = None,
):
    """组装并编译 LangGraph。

    runtime: LLMRuntime（含 rewrite_llm/chat_llm/embeddings/generator/rerank）。
             不传时用模块级 llm 单例（环境配置），向后兼容。
    store_:  VectorStore 实例。不传时用模块级 store 默认实例，向后兼容。
    """
    rt = runtime if runtime is not None else llm
    vs = store_ if store_ is not None else store
    # 取当前 runtime 的配置（LLMRuntime.settings）或旧 config 模块，字段名一致（大写）
    s = getattr(rt, "settings", None) or config

    # ---------- 节点 ----------

    def rewrite_node(state: RAGState) -> Dict:
        question = state["question"]
        resp = rt.rewrite_llm.invoke(_REWRITE_PROMPT.format(question=question))
        rewritten = (resp.content or "").strip()
        return {"rewritten": rewritten, "queries": [question, rewritten]}

    def hyde_node(state: RAGState) -> Dict:
        resp = rt.chat_llm.invoke(_HYDE_PROMPT.format(question=state["question"]))
    ...
    def retrieve_node(state: RAGState) -> Dict:
        ...
        for q in state["queries"]:
            if not q:
                continue
            merged.extend(vs.hybrid_search(q, top_k=s.HYBRID_FETCH_K))
        return {"retrieved": _dedup(merged)}
    ...
    def rerank_node(state: RAGState) -> Dict:
        ...
        idxs = rt.rerank(state["question"], texts, top_n=s.RERANK_TOP_K)
        return {"reranked": [docs[i] for i in idxs]}
