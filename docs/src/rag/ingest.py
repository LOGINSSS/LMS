...
class _Ctx:
    """入库上下文：把「模型运行时 / 向量库 / 配置」打包传递，支持 SDK 多实例。

    任一字段为 None 时回退到模块级默认（环境配置），保持向后兼容。
    """

    __slots__ = ("runtime", "store", "settings")

    def __init__(self, runtime=None, store=None, settings=None):
        self.runtime: LLMRuntime | None = runtime
        self.store: VectorStore | None = store
        self.settings: Settings | None = settings

    @property
    def embeddings(self):
        """embedding 客户端：优先 runtime，否则模块级 llm.embeddings。"""
        return (self.runtime if self.runtime is not None else llm).embeddings

    @property
    def generator(self):
        """生成模型（Qwen-VL）：优先 runtime，否则模块级 llm.generator。"""
        return (self.runtime if self.runtime is not None else llm).generator

    @property
    def vs(self):
        """向量库：优先注入实例，否则模块级 store（默认实例）。"""
        return self.store if self.store is not None else store

    @property
    def cfg(self):
        """配置：优先 Settings，否则旧 config 模块（字段名一致，大写）。"""
        return self.settings if self.settings is not None else config
...
def _pdf_to_markdown(path: Path, ctx: _Ctx | None = None) -> str:
    """用 MinerU 把 PDF 转 markdown（版面解析 + OCR + 公式 + 表格）。

    以子进程方式运行，把 PyTorch 模型隔离在独立进程里，不污染 FastAPI 进程。
    产出的 markdown 会继续走 split_text() 的结构化分片。
    """
    s = ctx.cfg if ctx is not None else config
    workdir = _mkdtemp("mineru_", Path(s.DATA_DIR))
    try:
        cmd = [
            _mineru_exe(),
            "-p", str(path),
        ...
        logger.info("MinerU 解析 %s (backend=%s, method=%s)", path.name, s.MINERU_BACKEND, s.MINERU_METHOD)
        subprocess.run(cmd, check=True, capture_output=True, text=True, timeout=s.MINERU_TIMEOUT)
...
def _save_pptx_picture(shape, pptx_path: Path, ctx: _Ctx | None = None):
    ...
    try:
        ...
        tmp = _mkdtemp("pptx_pic_", ctx.data_dir if ctx is not None else config.DATA_DIR)
        try:
...
def _picture_chunks(path: Path, ctx: _Ctx | None = None) -> List[Dict]:
    """单张图片 → [图片描述, 图片OCR] chunks（供入库/pptx 内嵌图片复用）。"""
    ocr_text = _ocr_image(path)
    description = _describe_image(path, ctx)
    chunks: List[Dict] = []
    if description:
        chunks.append(
            {
...
def _render_pptx_pages(pptx_path: Path, page_numbers: List[int], ctx: _Ctx | None = None) -> Dict[int, str]:
    ...
    if not page_numbers:
        return {}
    out_dir = _mkdtemp("pptx_render_", ctx.data_dir if ctx is not None else config.DATA_DIR)
    try:
...
def _extract_pptx(path: Path, ctx: _Ctx | None = None):
        ...
        for shape in _iter_pptx_shapes(slide.shapes):
            ...
            if st == MSO_SHAPE_TYPE.PICTURE:
                ...
                if w_px < _PPTX_IMAGE_MIN_PX or h_px < _PPTX_IMAGE_MIN_PX:
                    continue  # 装饰小图，丢弃
                png, tmpdir = _save_pptx_picture(shape, path, ctx)
                if png:
                    pic_tmp_dirs.append(tmpdir)
                    chunks.extend(_picture_chunks(png, ctx))
                    has_text = True
                continue
    ...
    if textless:
        rendered = _render_pptx_pages(path, textless, ctx)
...
def _pdf_pages_ocr(path: Path, ctx: _Ctx | None = None) -> str:
    ...
    try:
        ...
        tmp = _mkdtemp("pdf_ocr_", ctx.data_dir if ctx is not None else config.DATA_DIR)
        parts: List[str] = []
        try:
...
def _extract_text(path: Path, ctx: _Ctx | None = None) -> str:
    ...
    if ext == ".pptx":
        return _extract_pptx(path, ctx)[0]
    if ext == ".pdf":
        try:
            return _pdf_to_markdown(path, ctx)
        ...
        ocr_text = _pdf_pages_ocr(path, ctx)
        if ocr_text.strip():
...
def _describe_image(path: Path, ctx: _Ctx | None = None) -> str:
    """用 Qwen-VL 描述图片语义（DashScope API，无需本地模型）。"""
    b64 = base64.b64encode(path.read_bytes()).decode()
    gen = ctx.generator if ctx is not None else llm.generator
    msg = gen.invoke(
        [
            {
                "role": "user",
...
def ingest_image(
    ...
    *,
    ...
    runtime/store/settings 为 SDK 多实例注入参数（可省略，省略时用环境配置）。
    """
    chunks = _picture_chunks(path, _Ctx(runtime, store, settings))
    if not chunks:
        return 0
    return _embed_and_insert(
        chunks, source or path.name, "image", doc_id=doc_id, progress_cb=progress_cb,
        ctx=_Ctx(runtime, store, settings),
    )
...
def _embed_and_insert(
    ...
    *,
    ...
    vectors = ctx.embeddings.embed_documents(
        texts,
...
def ingest_text(
    ...
    *,
    ...
    """纯文本入库。runtime/store/settings 为 SDK 多实例注入参数（可省略）。"""
    ctx = _Ctx(runtime, store, settings)
    chunks = split_text(text)
    return _embed_and_insert(chunks, source, doc_type, ctx=ctx)
    ...
    *,
    ...
    if path.suffix.lower() in {".png", ".jpg", ".jpeg", ".bmp", ".webp"}:
        return ingest_image(path, source, doc_id=doc_id, progress_cb=progress_cb,
                            runtime=runtime, store=store, settings=settings)
    ...
    if path.suffix.lower() == ".docx":
        ...
        md_text, extra_chunks = _extract_pptx(path, ctx)
        chunks = split_text(md_text) + extra_chunks
    else:
        chunks = split_text(_extract_text(path, ctx))
    return _embed_and_insert(chunks, source, doc_type, doc_id=doc_id, progress_cb=progress_cb, ctx=ctx)
