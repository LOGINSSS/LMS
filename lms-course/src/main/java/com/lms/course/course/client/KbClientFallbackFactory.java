...
/**
 * lms-kb 自动建库失败降级：记录日志，不阻断建课主流程
 * （知识库可在后续手动补建，幂等 uk(owner_type, owner_id)）。
 */
@Slf4j
@Component
    ...
    public KbClient create(Throwable cause) {
        return new KbClient() {
            ...
            public R<Long> createKb(Map<String, Object> body) {
                log.warn("调用 lms-kb 自动建库失败 courseId={}: {}", body.get("courseId"), cause.getMessage());
                return R.fail("课程知识库创建失败");
            }

            @Override
            public R<Long> syncCourseText(Long courseId, Map<String, String> body) {
                log.warn("调用 lms-kb 正文同步失败 courseId={}: {}", courseId, cause.getMessage());
